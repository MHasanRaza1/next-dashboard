...
(async function syncTranslations() {
  ...
  for (const file of files) {
    ...
    const defaultExport = source.getExportAssignment(
      (exp) =>
        exp.getExpression().getExpression?.().getText() === "defineMessages"
    );
    ...
    for (const prop of props) {
      ...
      if (!urTranslations[fullKey]) {
        const msgObj = prop.getInitializerIfKind(
        ...
        if (
          defaultMessageProp &&
          defaultMessageProp.getKind() === SyntaxKind.PropertyAssignment
        ) {
          const defaultMessage = defaultMessageProp
            .getInitializer()
            ?.getText()
            ?.replace(/['"`]/g, "");
